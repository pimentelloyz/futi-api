    ...
    return {
      ...
      phases: league.phases.map((phase) => {
        ...
        return {
          ...
          type: phase.type,
          status: phase.status,
          teamsCount: phase.config?.teamsCount || null,
          groupsCount: phase.config?.groupsCount || null,
          teamsPerGroup: phase.config?.teamsPerGroup || null,
          hasHomeAway: phase.config?.hasHomeAway ?? phase.hasHomeAway,
          hasExtraTime: phase.config?.hasExtraTime ?? phase.hasExtraTime,
          hasPenalties: phase.config?.hasPenalties ?? phase.hasPenalties,
          advancingTeams: phase.config?.advancingTeams || null,
          advancingFrom: phase.config?.advancingFrom || null,
          tiebreakRules,
        };
      }),
